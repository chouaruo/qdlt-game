...
const OUTPUT_SCHEMA = {
  type: 'object',
  properties: {
    story: { type: 'string', minLength: 20, maxLength: 140 },
    choices: {
      type: 'array',
      items: {
        type: 'string',
...
${charCtx}
${traitCtx}
当前境界：${realm.name}（${realm.sub}），资产规模约${realm.asset}
玩家属性：本金=${state.money || 0}，心态=${state.mind || 0}，认知=${state.know || 0}，气运=${state.luck || 0}，回合=${state.turn || 0}
持仓：${Array.isArray(state.inv) && state.inv.length ? state.inv.join('、') : '空仓'}

请严格遵守返回结构：
1. story 用简体中文，100字内，延续上一段剧情，体现人物口吻。
2. choices 固定给3个选项，每个12字内，不要带编号。
3. effects 只填本回合即时变化；数值尽量克制，money 在 -50 到 100，mind/know 在 -20 到 20，luck 在 -15 到 15。
4. 没有物品变化时 item 和 lose_item 必须为 null。
 5. money 达到下一境界门槛 ${nextRealm.asset} 时 breakthrough 可为 true，否则通常为 false。`;
  ...
  return `${base}
7. 每回合只推进一个眼前场景，不要写成教程、设定集、目录、长篇总结、著书立说、数年后回顾。
8. story 禁止出现 Markdown 标题、代码块、列表、表格、JSON 示例说明。
9. choices 必须是正常中文操作选项，不能是单个字母、拼音残片、乱码或占位文本。`;
}
...
function normalizeResult(result) {
  ...
  normalized.effects.money = Math.max(-50, Math.min(100, normalized.effects.money));
