...
3.effects.money百分比整数(-50~100)，mind/know±15，luck±10。无物品变化item/lose_item=null。达${nextRealm.asset}可breakthrough=true。心态/认知扣减时story必须写明原因（如"看到血崩K线心态崩了""被假客服骗了"），不得无故扣减。
...
async function generateGameTurn({ userMsg, histSnap, state, byokKey, byokProvider, byokModel, byokBase }) {
  ...
  const apiBase = byokBase || API_BASE;
  const apiKey = byokKey || process.env.AI_API_KEY || process.env.ANTHROPIC_API_KEY;
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    const req = buildRequest(provider, model, apiBase, apiKey, sysPrompt, formatHistory(histSnap).concat({ role: 'user', content: userMsg }));
